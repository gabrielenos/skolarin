...
export default function SignupPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!signupRes.ok) {
        const message = signupData?.detail || "Daftar gagal"
        throw new Error(typeof message === "string" ? message : "Daftar gagal")
      }
      ...
      if (!loginRes.ok) {
        const message = loginData?.detail || "Login gagal"
      ...
      <div className="mx-auto w-full max-w-sm">
        <div className="mt-6 rounded-2xl bg-white p-6 shadow-sm">
          ...
          <form onSubmit={handleSubmit} className="mt-8 space-y-4">
            ...
            <label className="block">
              <span className="sr-only">Konfirmasi Kata Sandi</span>
              <div className="relative">
                <Lock className="absolute left-3 top-1/2 -translate-y-1/2 text-slate-400" />
                <input
            ...
            <div className="pt-2 text-center text-sm text-slate-500">
              Sudah punya akun?{" "}
              <Link href="/dashboard/login" className="font-semibold text-[#0B74E8] hover:underline">
                Masuk
              </Link>
            </div>
            ...
            <div className="pt-2">
              <div className="text-center text-xs text-slate-500">Terhubung dengan salah satu Opsi berikut</div>
