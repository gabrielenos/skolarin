...
export default function MathematicsPage() {
  ...
  const mathLevels = Array.from({ length: 30 }, (_, i) => ({
    id: i + 1,
    ...
    <div suppressHydrationWarning className={`min-h-screen flex flex-col ${pageBgClass}`}>
      ...
      <main className="flex-1 mx-auto w-full max-w-[600px] px-4 pt-6 pb-16 sm:px-6">
        <header className="text-center space-y-2 mb-8">
          ...
          >
            Home | Quiz Play | Guess The Word | Mathematics
          </p>
        </header>
        ...
        <section className="grid grid-cols-2 gap-3 max-w-[900px] mx-auto px-4 md:flex md:flex-col md:flex-wrap md:content-center md:max-h-[750px]">
          {mathLevels.map((level) => (
            <button
              key={level.id}
              type="button"
              onClick={() => router.push(`/guess_the_word/quiz?level=${level.id}&category=mathematics`)}
              className="focus:outline-none w-full md:w-[280px] lg:w-[320px] cursor-pointer"
            >
              <MathCard title={level.title} questions={level.questions} />
            </button>
          ))}
        </section>
      </main>
...
function MathCard({ title, questions }: { title: string; questions: number }) {
  return (
