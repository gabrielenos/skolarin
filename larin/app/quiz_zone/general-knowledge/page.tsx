...
export default function GeneralKnowledgePage() {
  ...
  return (
    <div suppressHydrationWarning className={`min-h-screen flex flex-col ${pageBgClass}`}>
      ...
      {/* Konten utama General Knowledge */}
      <main className="flex-1 mx-auto w-full max-w-[1363px] px-4 pt-10 pb-16 sm:px-6 lg:px-8">
        <header className="text-center space-y-2 mb-10">
          ...
          >
            Home | Quiz Play | Quiz Zone | General Knowledge
          </p>
          <p
        ...
        <section className="flex flex-col items-center gap-8">
          <div className="grid gap-6 lg:grid-cols-3">
            <GKCard title="Discovery" levels="Levels: 6" questions="Questions: 60" />
            <GKCard title="Art" levels="Levels: 6" questions="Questions: 59" />
            <GKCard title="Technology" levels="Levels: 7" questions="Questions: 70" />
          </div>
        </section>
      </main>
...
function GKCard({ title, levels, questions }: GKCardProps) {
  return (
    <div className="flex h-[150px] w-[400px] items-center rounded-[12px] border border-black/70 bg-[#1450A3] px-6 py-4 text-left text-white shadow-md">
      {/* Kolom kiri: ikon buku + bell seperti di Quiz Zone */}
      <div className="mr-4 flex flex-col items-center gap-2 flex-shrink-0">
        ...
        <div className="flex items-center gap-2 text-xs text-sky-100/90">
          <span>{levels}</span>
        </div>
      </div>

      {/* Kolom kanan: judul + Questions dengan bell */}
