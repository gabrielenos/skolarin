...
import { Settings, Trophy, ChevronDown, ChevronUp, Lock, ArrowLeft, LockOpen } from "lucide-react"
...
export default function MultipleChoicePage() {
  ...
  const [showAllMathLevels, setShowAllMathLevels] = useState(false)
  ...
  const getCategoryLevels = (category: Category) => {
    if (category.id === "matematika" && showAllMathLevels) {
                ...
                <div className="px-5 pb-5">
                  <div className="flex flex-wrap gap-3">
                    {getCategoryLevels(category).map((level) => (
                      <button
                        key={level}
                        onClick={() => level === 1 && router.push(`/quiz_zone/quiz?category=${category.id}&level=1`)}
                        className={`flex items-center gap-2 px-4 py-2 rounded-full transition-colors ${
                          level === 1
                            ? "bg-white text-slate-700 hover:bg-slate-50 cursor-pointer border border-slate-300"
                  ...
                  {category.id === "matematika" && !showAllMathLevels && category.levelCount > category.levels.length && (
                    <button 
                      onClick={() => setShowAllMathLevels(true)}
